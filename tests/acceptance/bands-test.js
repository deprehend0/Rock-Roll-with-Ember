...
test('Sort songs in various ways', function(assert) {
  ...
  andThen(function() {
    ...
    assertTrimmedText(assert, '.song:first', 'Elephants', 'The first song ' +
      'is the highest ranked, first in the alphabet');
    assertTrimmedText(assert, '.song:last', 'New Fang', 'The last song is ' +
      'the lowest ranked, last in the alphabet');
  });
...
test('Search songs', function(assert) {
  ...
  andThen(function() {
    assertLength(assert, '.song', 2, 'The songs matching the search term ' +
      'are displayed');
  });

  click('button.sort-title-desc');
  andThen(function() {
    assertTrimmedText(assert, '.song:first', 'No One Loves Me & Neither ' +
      'Do I', 'A matching song that comes later in the alphabet appears on ' +
      'top');
